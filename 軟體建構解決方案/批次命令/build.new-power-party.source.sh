	...
	cp "${FILE_SOURCE_DESIGN}" "${new_source_file}_with_li"
	xmlstarlet edit --pf --ps --inplace --update "//_:g[@inkscape:label='官方版本（LION 版v2）']/@style" --value "display:none" "${new_source_file}_with_li"
	xmlstarlet edit --pf --ps --inplace --update "//_:g[@inkscape:label='台澎琉蘭綠東南沙底圖']/@style" --value "display:inline" "${new_source_file}_with_li"
	sed --in-place s/18a303/f7bf03/g "${new_source_file}_with_li"
	xmlstarlet edit --pf --ps --inplace --update "//_:g[@inkscape:label='時代力量黨版']/@style" --value "display:inline" "${new_source_file}_with_li"
	inkscape --export-png="${DIRECTORY_BUILD_ARTIFACTS}/$(basename --suffix=.svg "${new_source_file}_with_li").png" "${new_source_file}_with_li"
	
	declare new_source_file_with_li_white_background="${DIRECTORY_BUILD_ARTIFACTS}/$(basename --suffix=.svg "${new_source_file}_with_li")-background-white.svg"
	printf "資訊：正在建構時代力量版白背景版……\n"
	cp "${new_source_file}_with_li" "${new_source_file}_with_li_white_background"
	xmlstarlet edit --pf --ps --inplace --update "//@inkscape:pageopacity" --value 1 "${new_source_file}_with_li_white_background"
	inkscape --export-png="${DIRECTORY_BUILD_ARTIFACTS}/$(basename --suffix=.svg "${new_source_file}_with_li_white_background").png" "${new_source_file}_with_li_white_background"
