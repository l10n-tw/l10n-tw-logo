	declare new_source_file="$DIRECTORY_BUILD_ARTIFACTS/$(basename --suffix=.svg "$FILE_SOURCE_DESIGN")-china-communist.svg"
	printf "資訊：正在建構中華人民共和國共產黨版……\n"
	...
	xmlstarlet edit --pf --ps --inplace --update "//_:g[@inkscape:label='中華人民共和國共產黨版']/@style" --value "display:inline" "$new_source_file"
	inkscape --export-png="$DIRECTORY_BUILD_ARTIFACTS/$(basename --suffix=.svg "$new_source_file").png" "$new_source_file"
	
	declare new_source_file_white_background="$DIRECTORY_BUILD_ARTIFACTS/$(basename --suffix=.svg "$FILE_SOURCE_DESIGN")-china-communist-background-white.svg"
	printf "資訊：正在建構中華人民共和國共產黨版白背景版……\n"
